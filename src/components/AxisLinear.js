...
class Axis extends React.Component {
  ...
  componentDidUpdate(oldProps, oldState) {
    ...
    if (
      ...
      this.props.axis !== oldProps.axis ||
      oldState.rotation !== this.state.rotation
    ) {
      this.measure();
    }
  }
  render() {
    ...
    if (vertical) {
      if (position === positionLeft) {
        axisPath = `M ${-tickSizeOuter} ${range0} H 0 V ${range1} H ${-tickSizeOuter}`;
      } else {
        axisPath = `M ${tickSizeOuter} ${range0} H 0 V ${range1} H ${tickSizeOuter}`;
      }
    } else if (position === positionBottom) {
      axisPath = `M 0 ${tickSizeOuter} V 0 H ${range1} V ${tickSizeOuter}`;
    } else {
      axisPath = ` M 0 ${-tickSizeOuter} V 0 H ${range1} V ${-tickSizeOuter}`;
    }
