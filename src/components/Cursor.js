...
class Cursor extends React.PureComponent {
  ...
  updateCursor = () => {
    ...
    const resolvedAxisID =
      ...
      (datum &&
        datum.series &&
        datum.series[`${primary ? "primary" : "secondary"}AxisID`]);
  ...
  render() {
    ...
    const { axis, siblingAxis, value, datum } = cursor;
    ...
    resolvedValue = Utils.isValidPoint(resolvedValue)
      ? resolvedValue
      : this.prevValue;
    ...
    const start = {
      transform: `translate(${left + gridX}px, ${top + gridY}px)`,
      opacity: 0
    };
        ...
        {showLine ? (
          <Div
            style={{
              ...
              top: 0,
              left: 0,
              transform: `translate(${lineStartX}px, ${lineStartY}px)`,
              width: `${lineWidth}px`,
          ...
          >
            ...
            <Div
              style={{
                ...
                borderRadius: "3px",
                position: "relative",
                transform: `translate(${alignPctX}%, ${alignPctY}%)`,
